/*
 * myApps.cc — application / traffic class.
 * Reused from FlyNetSim (https://github.com/saburhb/FlyNetSim, ACM MSWiM 2018),
 * with small additions for the defense send/receive counters.
 * Distributed under the GNU GPL v2. See LICENSE and NOTICE at the repo root.
 */
#include <iostream>
#include <fstream>
#include <string>
...
{
  if (send_socket == 0 || m_socket == 0) { free(msg); return; }
  int len = strlen(msg);
  pthread_mutex_lock(&tNext_mutex);
  Ptr<Packet> packet = Create<Packet> ((uint8_t*)msg, len);
